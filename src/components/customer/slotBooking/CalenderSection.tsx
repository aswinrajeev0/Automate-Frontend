...
const CalenderSection: React.FC<CalenderSectionProps> = ({ setBookingSubmitted }) => {
    ...
    // Check if a slot is expired (past the current time)
    const isSlotExpired = useCallback((date: Date, startTime: string): boolean => {
        const now = new Date();
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`
                                                            ...
                                                            ${!slot.isAvailable || isExpired ? "bg-red-100 text-red-800 cursor-not-allowed" : selectedSlot?._id !== slot._id ? "bg-gray-100 hover:bg-gray-200" : ""}
                                                            focus:outline-none focus:ring-2 focus:ring-blue-500
                                                        `}
                                                        onClick={() => slot.isAvailable && !isExpired && handleSlotSelection(slot)}
                                                        disabled={!slot.isAvailable || isExpired}
                                                        aria-label={`Book time slot at ${formatTimeDisplay(slot.startTime)}`}
                                                        aria-pressed={selectedSlot?._id === slot._id}
                                                        aria-disabled={!slot.isAvailable || isExpired}
                                                    >
                                                        <div>{formatTimeDisplay(slot.startTime)}</div>
                                                        <div className="text-xs mt-1">
                                                            {!slot.isAvailable ? "Fully Booked" : isExpired ? "Expired" : `${serviceDuration} hour${serviceDuration > 1 ? "s" : ""}`}
                                                        </div>
                                                        {slot.currentBookings > 0 && slot.isAvailable && !isExpired && (
